# PROBLEM
#
# The four adjacent digits in the 1000-digit number that have the greatest product are 9 * 9 * 8 * 9 = 5832.
# Find the thirteen adjacent digits in the 1000-digit number that have the greatest product.
# What is the value of this product?

# PSEUDO
#
# iterate through the list and check slices of 13 numbers
# if it has a zero in it, pass to the next one
# if not, multiply them out, check against existing 'best'
# at the end, return the greatest product

num_string = ''
# print(num_string)
num_list = list(num_string)
print(num_list)

# testing my slicing method
print(num_list[0:13])
product = 1
for num in num_list[0:13]:
    if num == 0:
        break
    product *= int(num)


# Solution
def greatest_product(num_list):
    best = 0
    for i in range(len(num_list) - 13):
        product = 1
        # Take a slice of 13 strings from the list
        test_list = num_list[(0 + i):(13 + i)]
        for num in test_list:
            if num == '0':
                # Exit loop early if there is a zero
                break
            else:
                # calculate the product of multiplying all numbers in the list
                product *= int(num)
        if product > best:
            # keep track of the best combination
            best = product
    return best

print(greatest_product(num_list))